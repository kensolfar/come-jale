  ...
  return (
    <div style={{ padding: 0 }}>
      ...
      <form onSubmit={handleSubmit} style={{ display: 'flex', gap: 12, marginBottom: 24, flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        {edit && <button type="button" onClick={() => { setEdit(null); setForm({ nombre: '', tipo: 'SERVICIO', monto: '', porcentaje: '', es_impuesto: false }); }} style={{ background: '#232428', color: '#fff', border: '1.5px solid #8DAA91', borderRadius: 8, padding: '7px 16px', fontWeight: 700, fontSize: 15, transition: 'background 0.2s', cursor: 'pointer' }}>{t('Cancelar')}</button>}
      </form>
      <table style={{ width: '100%', background: '#18191b', color: '#fff', borderRadius: 8, overflow: 'hidden', fontSize: 15 }}>
        <thead>
          <tr style={{ background: '#232428', color: '#8DAA91' }}>
            ...
            <th style={{ padding: '10px 8px' }}>{t('Porcentaje')}</th>
            <th style={{ padding: '10px 8px' }}>{t('¿Es impuesto?')}</th>
            <th></th>
          </tr>
        </thead>
        <tbody>
          {cargos.map(cargo => (
            <tr key={cargo.id} style={{ borderBottom: '1px solid #232428' }}>
              ...
              <td style={{ padding: '8px 8px' }}>{cargo.porcentaje || '-'}</td>
              <td style={{ padding: '8px 8px' }}>{cargo.es_impuesto ? t('Sí') : t('No')}</td>
